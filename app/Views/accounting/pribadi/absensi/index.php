<?php
// View: accounting/pribadi/absensi/index.php
// Tampilan identik dengan admin/pribadi/absensi.php
?>
...
<div class="container-fluid px-3 px-md-4 py-4">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-2">
        <div>
            ...
            <small class="text-muted">Pencatatan kehadiran harian karyawan yang terhubung secara otomatis ke sistem Monitoring Direktur.</small>
        </div>
    </div>
    ...
    <div class="row g-4">
        ...
        <div class="col-lg-5 col-xl-4">
            <div class="card border-0 rounded-4 shadow-lg overflow-hidden h-100">
                <div class="card-header text-white text-center py-3 px-4" style="background: linear-gradient(135deg, #1e3c72, #2a5298);">
                ...
                <div class="card-body p-4 text-center d-flex flex-column justify-content-center">
                    <div class="mb-3">
                        <span class="badge bg-primary bg-opacity-10 text-primary px-3 py-2 rounded-pill fw-semibold mb-2 d-inline-block">
                            <i class="fas fa-user me-1"></i> <?= esc(session()->get('name') ?? 'Accounting Staff') ?>
                        </span>
                        <div class="text-xs text-muted">Keuangan &amp; Akuntansi &bull; Accounting Staff</div>
                    </div>

                    <!-- Jam Digital Live -->
                    <div class="p-3 rounded-4 bg-light border mb-4 shadow-inner">
                        <h2 class="fw-bold text-dark mb-0 font-monospace" id="cardClock" style="font-size: 2.4rem; letter-spacing: 2px;">
                    ...
                    <div class="mb-4">
                        ...
                        <?php else: ?>
                            <span class="badge bg-info bg-opacity-10 text-info border border-info border-opacity-25 px-3 py-2 rounded-pill fw-semibold">
                    ...
                    <div class="d-grid gap-2">
                        <?php if (empty($todayAbsen)): ?>
                            <form action="<?= site_url('accounting/pribadi/checkin') ?>" method="POST"
                                  onsubmit="const btn=this.querySelector('button');if(btn){btn.disabled=true;btn.innerHTML='<i class=\'fas fa-spinner fa-spin me-1\'></i> Mencatat...';}">
                                <?= csrf_field() ?>
                                <button type="submit" class="btn btn-success rounded-pill py-2 fw-bold w-100 shadow-sm">
                                    <i class="fas fa-sign-in-alt me-2"></i> Absen Masuk (Check-In)
                                </button>
                            </form>
                            <button type="button" class="btn btn-secondary rounded-pill py-2 fw-bold w-100 opacity-50" disabled>
                                <i class="fas fa-sign-out-alt me-2"></i> Absen Pulang (Check-Out)
                            </button>
                        <?php elseif (empty($todayAbsen['waktu_pulang'])): ?>
                            <button type="button" class="btn btn-success rounded-pill py-2 fw-bold w-100 opacity-50" disabled>
                                <i class="fas fa-check me-2"></i> Sudah Check-In (<?= date('H:i', strtotime($todayAbsen['waktu_masuk'])) ?>)
                            </button>
                            <form action="<?= site_url('accounting/pribadi/checkout') ?>" method="POST"
                                  onsubmit="const btn=this.querySelector('button');if(btn){btn.disabled=true;btn.innerHTML='<i class=\'fas fa-spinner fa-spin me-1\'></i> Mencatat...';}">
                                <?= csrf_field() ?>
                                <button type="submit" class="btn btn-danger rounded-pill py-2 fw-bold w-100 shadow-sm">
                                    <i class="fas fa-sign-out-alt me-2"></i> Absen Pulang (Check-Out)
                                </button>
                            </form>
                        <?php else: ?>
                            <button type="button" class="btn btn-outline-success rounded-pill py-2 fw-bold w-100" disabled>
                        ...
                        <table class="table table-hover align-middle mb-0 text-sm">
                            ...
                            <tbody>
                                ...
                                <?php foreach ($riwayatAbsensi as $r):
                                    ...
                                    if (in_array($st, ['izin', 'sakit'])) $badgeClass = 'bg-info text-white';
                                    if ($st === 'alpha') $badgeClass = 'bg-danger text-white';
                                ?>
                                <tr>
                                    <td class="ps-4 fw-bold text-dark"><?= date('d/m/Y', strtotime($r['tanggal'])) ?></td>
                                    ...
                                    <td>
                                        <div class="text-xs text-dark text-truncate" style="max-width:200px;" title="<?= esc($r['keterangan'] ?? '') ?>">
                                            <?= esc($r['keterangan'] ?? 'Kantor CDW Engineering') ?>
                                        </div>
                                    </td>
...
function updateClock() {
    ...
    if (el) el.innerHTML = `${h}:${m}:${s} <span class="fs-6 fw-normal text-muted">WIB</span>`;
}
