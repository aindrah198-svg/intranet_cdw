...
$existingImgUrl = getGudangImgUrlAdmin($g['foto_barang'] ?? '');
?>
    ...
    <div class="row justify-content-center">
        <div class="col-lg-9">
            <div class="card border-0 rounded-4 shadow-lg overflow-hidden mb-4">
                ...
                <form action="<?= base_url('admin/inventaris/gudang/update') ?>" method="POST" enctype="multipart/form-data" onsubmit="const btn = this.querySelector('button[type=submit]'); if(btn){ btn.disabled = true; btn.innerHTML = '<i class=\'fas fa-spinner fa-spin me-1\'></i> Menyimpan...'; }">
                    ...
                    <div class="card-body p-4">
                        ...
                        <div class="mb-3">
                            ...
                            <?php if (!empty($existingImgUrl)): ?>
                                <div class="mb-2">
                                    <a href="<?= $existingImgUrl ?>" target="_blank">
                                        <img src="<?= $existingImgUrl ?>" alt="Foto Material" class="rounded-3 border shadow-sm" style="max-height: 140px; object-fit: contain;">
                                    </a>
                                    <small class="text-muted d-block mt-1">Foto terpasang saat ini</small>
                                </div>
                            <?php endif; ?>
                            <input type="file" class="form-control rounded-3" name="foto_barang" accept="image/*">
                            <small class="text-muted text-xs d-block mt-1">
                                <i class="fas fa-compress-alt me-1 text-success"></i> Upload baru jika ingin mengganti foto. Foto akan dikompresi otomatis oleh sistem.
                            </small>
                        </div>
