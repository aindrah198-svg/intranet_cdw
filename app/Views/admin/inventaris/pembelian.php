...
<div class="container-fluid py-3 py-md-4">
    ...
    <div class="d-flex justify-content-between align-items-center bg-white rounded-3 shadow-sm p-3 mb-3 border border-light flex-wrap gap-3">
        ...
        <div class="d-flex gap-2 flex-wrap">
            <a href="<?= base_url('admin/inventaris/pembelian/tambah') ?>" class="btn btn-primary rounded-pill shadow-sm d-inline-flex align-items-center fw-semibold">
                <i class="fas fa-plus me-1.5"></i> Buat Pencatatan PR Baru
            </a>
            <a href="<?= base_url('admin/inventaris/pembelian/export-excel') ?>" class="btn btn-outline-success rounded-pill shadow-sm d-inline-flex align-items-center fw-semibold">
                <i class="fas fa-file-excel me-1.5"></i> Export Excel
            </a>
            <a href="<?= base_url('admin/inventaris/pembelian/cetak') ?>" target="_blank" class="btn btn-outline-info rounded-pill shadow-sm d-inline-flex align-items-center fw-semibold">
                    ...
                    <?php if (!empty($pembelian)): ?>
                        <?php $no = 1; foreach ($pembelian as $p): ?>
                            <tr>
                                ...
                                <td>
                                    <strong class="text-dark d-block"><?= esc($p['nama_lengkap'] ?? 'Admin Panel') ?></strong>
                                ...
                                <td class="text-center">
                                    <?php
                                        ...
                                        if (strpos($bayar, 'lunas') !== false || strpos($bayar, 'sudah') !== false) $badgeBayar = 'bg-success text-white';
                                    ?>
                                ...
                                <td class="text-center">
                                    <?php
                                        ...
                                        elseif (strpos($terima, 'proses') !== false || strpos($terima, 'dikirim') !== false) $badgeTerima = 'bg-info text-white';
                                    ?>
                                ...
                                <td class="text-center pe-3">
                                    <div class="btn-group btn-group-sm">
                                        <a href="<?= base_url('admin/inventaris/pembelian/detail/' . $p['id']) ?>" class="btn btn-outline-primary rounded-start px-2" title="Detail">
                                            <i class="fas fa-eye"></i>
                                        </a>
                                        <a href="<?= base_url('admin/inventaris/pembelian/edit/' . $p['id']) ?>" class="btn btn-outline-secondary px-2" title="Edit">
                                            <i class="fas fa-edit"></i>
                                        </a>
                                        <a href="<?= base_url('admin/inventaris/pembelian/cetak/' . $p['id']) ?>" target="_blank" class="btn btn-outline-info px-2" title="Cetak PR">
                                            <i class="fas fa-print"></i>
                                        </a>
                                        <form action="<?= base_url('admin/inventaris/pembelian/delete/' . $p['id']) ?>" method="POST" class="d-inline" onsubmit="return confirm('Hapus pencatatan PR ini?')">
