<?php
$data = [
    'title'  => 'Detail & Edit Checklist Tugas Saya',
    'active' => 'tugas-saya',
...
$isAllCompleted = ($totalItems > 0 && $completedItems === $totalItems) || ($totalItems === 0 && $t['status'] === 'selesai');
?>
...
<div class="container-fluid px-3 px-md-4 py-4">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-2">
        <div>
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb mb-1 text-xs">
                    ...
                    <li class="breadcrumb-item"><a href="<?= base_url('admin/tugas-saya') ?>" class="text-decoration-none text-muted">Tugas Hari Ini</a></li>
        ...
        <div class="d-flex gap-2">
            <a href="<?= base_url('admin/tugas-saya') ?>" class="btn btn-outline-secondary rounded-pill px-3 text-sm font-semibold">
                <i class="fas fa-arrow-left me-1.5"></i> Kembali
            </a>
            <?php if($isAllCompleted): ?>
                <a href="<?= base_url('admin/tugas-saya/buat-laporan/'.$t['id']) ?>" class="btn btn-success rounded-pill px-4 text-sm font-semibold shadow-sm">
                    <i class="fas fa-file-export me-1.5"></i> Buat Menjadi Laporan Harian
                </a>
            <?php else: ?>
                <button type="button" class="btn btn-secondary rounded-pill px-4 text-sm font-semibold opacity-60" onclick="alertNotCompleted()">
                    <i class="fas fa-lock me-1.5"></i> Buat Menjadi Laporan Harian
                </button>
            <?php endif; ?>
        </div>
    </div>
            ...
            <div class="card border-0 rounded-4 shadow-sm overflow-hidden mb-4">
                <div class="card-header bg-white py-3 px-4 d-flex align-items-center justify-content-between border-bottom border-light">
                    ...
                    <span class="badge bg-primary px-3 py-1.5 rounded-pill text-xs fw-semibold">
                        STATUS UTAMA: <?= strtoupper(esc($t['status'] === 'proses' ? 'Sedang Dikerjakan' : $t['status'])) ?>
                    </span>
                </div>
                <div class="card-body p-4">
                    <div class="mb-4">
                        <label class="text-muted text-xs font-semibold uppercase mb-1">Catatan & Arahan Direktur</label>
                    ...
                    <!-- Checklist Sub-Item Form Update -->
                    <form action="<?= base_url('admin/tugas-saya/update-subitems/'.$t['id']) ?>" method="POST">
                        <?= csrf_field() ?>
                        ...
                        <div class="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
                            ...
                            <div class="d-flex gap-2">
                                <button type="submit" name="mark_all_completed" value="1" class="btn btn-sm btn-outline-success rounded-pill px-3 font-semibold text-xs">
                                    ...
                                    <?php else: ?>
                                        ...
                                        <tr>
                                            ...
                                            <td class="text-center">
                                                <select name="subitem_status[<?= $item['id'] ?>]" class="form-select form-select-sm rounded-pill text-xs fw-bold text-center <?= $item['status_item']==='selesai'?'border-success text-success':'border-warning text-dark' ?>">
                        ...
                        <div class="d-flex justify-content-between align-items-center flex-wrap gap-2 mt-3">
                            ...
                            <button type="submit" class="btn btn-primary rounded-pill px-4 font-semibold shadow-sm text-sm">
                                <i class="fas fa-save me-1.5"></i> Simpan Perubahan Sub-Item
                            </button>
                        </div>
                    </form>
                </div>
        ...
        <div class="col-lg-4">
            <div class="card border-0 rounded-4 shadow-sm overflow-hidden mb-4">
                ...
                <div class="card-body p-4 text-sm">
                    <div class="mb-3">
                        <small class="text-muted text-xs font-semibold uppercase d-block">Progres Pemenuhan Checklist</small>
                    ...
                    <div class="mb-3">
                        <small class="text-muted text-xs font-semibold uppercase d-block">Tanggal Tugas</small>
                    ...
                    <div class="mb-3">
                        <small class="text-muted text-xs font-semibold uppercase d-block">Tenggat Jam</small>
                        <div class="fw-bold text-danger mt-1"><i class="far fa-clock me-1"></i><?= date('H:i', strtotime($t['tenggat_waktu'] ?: '17:00')) ?> WIB</div>
                    </div>

                    <hr>

                    <form action="<?= base_url('admin/tugas-saya/update-status/'.$t['id']) ?>" method="POST" class="mb-3">
                    ...
                    <div class="d-grid mt-3">
                        <?php if($isAllCompleted): ?>
                            <a href="<?= base_url('admin/tugas-saya/buat-laporan/'.$t['id']) ?>" class="btn btn-success rounded-pill font-semibold shadow-sm text-sm">
                                <i class="fas fa-file-export me-1.5"></i> Buat Laporan Harian
                            </a>
                        <?php else: ?>
                            <button type="button" class="btn btn-secondary rounded-pill font-semibold text-sm opacity-60" onclick="alertNotCompleted()">
                                <i class="fas fa-lock me-1.5"></i> Buat Laporan Harian (Terkunci)
                            </button>
                        <?php endif; ?>
                    </div>
                </div>
