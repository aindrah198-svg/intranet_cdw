...
$projects = $projects ?? [];
$projectsPending = $projects_pending ?? [];
?>

<style>
    /* Styling Premium Modern Material & Glassmorphism (Sama Dengan Direktur) */
            ...
            <?php foreach($projects as $p): ?>
                ...
                <div class="col-12 col-xl-6 timeline-card-wrapper">
                    <div class="card employee-card-modern p-3 p-sm-4 h-100">
                        ...
                        <!-- Card Footer Action Buttons (Detail & Task, Excel, Cetak PDF, Selesai, Hapus) -->
                        <div class="pt-3 mt-auto border-top border-light d-flex align-items-center justify-content-end gap-1.5 flex-wrap">
                            <a href="<?= base_url('admin/timeline-kerja/detail/'.$p['id']) ?>" class="btn-action-pill btn-action-view" title="Detail Timeline & Schedule">
...
<div class="modal fade" id="modalAktifkanProyek" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 rounded-4 shadow-lg">
            ...
            <form action="<?= base_url('admin/timeline-kerja/aktifkan') ?>" method="POST">
                <div class="modal-body p-4">
                    <p class="text-muted text-xs mb-3">Pilih proyek yang diinisiasi untuk mulai disusun jadwal pelaksanaan timelinenya secara harian/mingguan.</p>
...
<script>
    ...
    function confirmDeleteTimeline(id, nama) {
        Swal.fire({
            ...
            text: 'Jadwal timeline "' + nama + '" akan dikosongkan. Data proyek tetap aman dan utuh.',
            icon: 'warning',
            showCancelButton: true,
            ...
            if (result.isConfirmed) {
                const form = document.createElement('form');
                form.method = 'POST';
                form.action = '<?= base_url('admin/timeline-kerja/delete') ?>/' + id;
                document.body.appendChild(form);
                form.submit();
            }
        });
