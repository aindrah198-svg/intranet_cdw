...
$jenisStr  = $surat['jenis_surat']      ?? 'SURAT PERUSAHAAN';

/* ── helper: render isi_surat (HTML tables pass through, plain text gets formatted) ── */
function adminFormatBodyTextWithTables($text) {
    ...
    foreach ($parts as $part) {
        if (preg_match('/^<table/i', trim($part))) {
            $output .= $part; // raw HTML table – render as-is
        } else {
            $lines     = explode("\n", $part);
...
<style>
    ...
    .status-pill { display:inline-flex;align-items:center;padding:5px 14px;border-radius:30px;font-size:0.8rem;font-weight:600;letter-spacing:0.3px; }
    .status-pill-active   { background:rgba(25,135,84,0.12);color:#198754;border:1px solid rgba(25,135,84,0.25); }
    .status-pill-inactive { background:rgba(220,53,69,0.12);color:#dc3545;border:1px solid rgba(220,53,69,0.25); }
    .status-pill-draft    { background:rgba(255,193,7,0.15);color:#b58100;border:1px solid rgba(255,193,7,0.3); }
    .data-pill-bar { background:#f8fafc;border:1px solid rgba(226,232,240,0.9);border-radius:12px;padding:10px 14px;margin-bottom:12px; }
    .data-label { font-size:0.7rem;font-weight:700;text-transform:uppercase;letter-spacing:0.5px;color:#64748b;margin-bottom:2px;display:flex;align-items:center;gap:6px; }
    .data-value { font-size:0.92rem;font-weight:600;color:#0f172a; }
    ...
    .blue-header-banner { background:#1e40af;color:#ffffff;padding:10px 16px;font-weight:700;letter-spacing:1px;text-transform:uppercase;border-radius:4px;margin-bottom:14px; }

    /* Tables inside letter body */
    .custom-doc-table { width:100%;border-collapse:collapse;margin:12px 0;font-size:0.86rem; }
    .custom-doc-table th, .custom-doc-table td { padding:7px 10px;border:1px solid #cbd5e1; }
    .table-style-blue_header th { background:#1e40af;color:#ffffff;font-weight:700;text-transform:uppercase;border-color:#1e3a8a; }
    .table-style-gold_header th { background:#d97706;color:#ffffff;font-weight:700;text-transform:uppercase;border-color:#b45309; }
    ...
    /* Ensure tables don't overflow narrow containers in preview */
...
<div class="container-fluid py-3 py-md-4">
    ...
    <div class="d-flex justify-content-between align-items-center bg-white rounded-3 shadow-sm p-3 mb-4 border border-light flex-wrap gap-2 no-print">
        <div class="d-flex align-items-center">
            <div class="text-white rounded-3 p-2 me-3 d-flex align-items-center justify-content-center shadow-sm" style="width:46px;height:46px;background:linear-gradient(135deg,#1e3c72 0%,#2a5298 100%);">
        ...
        <div class="d-flex align-items-center gap-2 flex-wrap">
            ...
            <a href="<?= base_url('admin/surat') ?>" class="btn btn-outline-secondary rounded-pill px-3 py-2 shadow-sm d-inline-flex align-items-center text-sm fw-semibold">
    ...
    <div class="row g-4">
        ...
        <div class="col-12 col-lg-4 no-print">
            ...
            <div class="card employee-card-modern p-4 mb-4">
                ...
                <div class="data-pill-bar">
                    ...
                    <div class="data-value"><?= !empty($surat['jenis_surat']) ? esc($surat['jenis_surat']) : esc($jenisStr) ?></div>
                </div>
                <div class="data-pill-bar">
                    <div class="data-label"><i class="far fa-file-alt text-primary"></i> Ukuran Kertas</div>
                ...
                <div class="data-pill-bar mb-3">
                    ...
                    <div class="data-value">
                        <?php
                            ...
                            $statusText = 'Draft';
                            if ($statusStr === 'diterbitkan') { $statusPillClass = 'status-pill-active'; $statusText = 'Diterbitkan'; }
                            elseif ($statusStr === 'dibatalkan') { $statusPillClass = 'status-pill-inactive'; $statusText = 'Dibatalkan'; }
                        ?>
                ...
                <!-- Form Ubah Status -->
                <form action="<?= base_url('admin/surat/update-status/' . $surat['id']) ?>" method="post" class="mt-3">
                    <?= csrf_field() ?>
                    <label class="form-label fw-bold text-dark" style="font-size:0.8rem;">Ubah Status Dokumen</label>
            ...
            <?php if (!empty($karyawan)): ?>
                <div class="card employee-card-modern p-4">
                    <h6 class="fw-bold text-dark mb-3 d-flex align-items-center gap-2">
                        <i class="fas fa-user-tie text-primary"></i> Penerima Surat
                    </h6>
                    <div class="d-flex align-items-center gap-3 mb-3">
                        <div class="flex-shrink-0" style="width:48px;height:48px;background:linear-gradient(135deg,#1e3c72 0%,#2a5298 100%);color:#fff;border-radius:50%;display:flex;align-items:center;justify-content:center;font-weight:700;font-size:1.2rem;">
                    ...
                    <div class="data-pill-bar mb-2">
                        <div class="data-label"><i class="fas fa-id-card text-primary"></i> NIK</div>
        ...
        <!-- Render Kertas Surat Preview – Always from PHP/isi_surat (clean, no duplicate) -->
                ...
                <div class="doc-page-sheet">
                    ...
                    <div style="position:relative;z-index:2;">
                        ...
                        <?php elseif ($accent === 'blue_bar'): ?>
                            <div class="blue-header-banner text-center my-3"><?= esc($jenisStr) ?></div>
                        <?php endif; ?>

                        <!-- Title -->
                        <?php if ($accent !== 'blue_bar'): ?>
                            <h5 class="text-center fw-bold text-dark text-uppercase mb-3" style="letter-spacing:0.5px;"><?= esc($jenisStr) ?></h5>
                        <?php endif; ?>

                        <!-- Nomor & Tanggal -->
                        <div class="d-flex justify-content-between fw-semibold text-secondary mb-3 pb-2 border-bottom border-light" style="font-size:0.88rem;">
                        ...
                        <!-- Isi Surat – render HTML tables properly -->
                        <div class="text-dark mb-4 page-content-area" style="line-height:1.8;flex-grow:1;font-size:0.92rem;">
                            <?= adminFormatBodyTextWithTables($surat['isi_surat'] ?? '') ?>
                        </div>

                        <!-- Catatan Internal & Tanda Tangan -->
                        <div style="margin-top:auto;padding-top:15px;">
                            <?php if (!empty($surat['catatan'])): ?>
                                <div class="alert alert-warning border-0 rounded-3 text-dark p-2 mb-3 no-print" style="font-size:0.78rem;">
                                    <i class="fas fa-exclamation-circle me-1 text-warning"></i>
                                    <strong>Catatan Internal:</strong> <?= esc($surat['catatan']) ?>
                                </div>
                            <?php endif; ?>
                            <div class="row pt-2 text-center" style="position:relative;z-index:5;margin-bottom:<?= ($addrPos === 'footer' || $accent === 'yellow_corner') ? '45px' : '10px' ?>;">
                                <div class="col-6 ms-auto">
                                    ...
                                    <p class="fw-bold text-dark mb-0 border-bottom d-inline-block pb-1" style="min-width:150px;">Direktur Utama</p>
                                </div>
                            </div>
                        </div>
...
<div class="modal fade" id="confirmPrintModal" tabindex="-1" aria-hidden="true" style="z-index:10999;">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content rounded-4 border-0 shadow-lg" style="overflow:hidden;">
            <div class="modal-header text-white border-0 py-3" style="background:linear-gradient(135deg,#1e3c72 0%,#2a5298 100%);">
            ...
            <div class="modal-body p-4 text-center">
                <div class="mb-3 text-primary"><i class="fas fa-file-invoice fa-4x"></i></div>
                <h5 class="fw-bold text-dark mb-2">Siap untuk Pratinjau & Cetak?</h5>
                <p class="text-muted mb-3" style="font-size:0.9rem;">Sistem akan menampilkan halaman pratinjau cetak sebelum disimpan sebagai PDF.</p>
                <div class="p-3 bg-light rounded-3 text-start border mb-3" style="font-size:0.8rem;line-height:1.6;">
                    <div class="d-flex justify-content-between mb-1"><span class="text-muted">Jenis Surat:</span><strong class="text-dark"><?= esc($jenisStr) ?></strong></div>
                    <div class="d-flex justify-content-between mb-1"><span class="text-muted">Penerima:</span><strong class="text-dark"><?= esc($surat['nama_lengkap'] ?? $karyawan['nama_lengkap'] ?? '-') ?></strong></div>
                    <div class="d-flex justify-content-between mb-1"><span class="text-muted">Tanggal:</span><strong class="text-dark"><?= date('d F Y', strtotime($surat['tanggal_surat'])) ?></strong></div>
                    <div class="d-flex justify-content-between"><span class="text-muted">Format Kertas:</span><strong class="text-dark"><?= esc($paperSize) ?></strong></div>
                </div>
            </div>
...
window.addEventListener('load', function() {
    ...
    if (urlParams.has('print_now')) {
        ...
        if (typeof Swal !== 'undefined') {
            Swal.fire({ icon:'success', title:'Tersimpan!', text:'Membuka pratinjau cetak...', timer:1500, showConfirmButton:false,
                willClose: () => { proceedToDedicatedPrintView(); }
            });
        } else {
            setTimeout(proceedToDedicatedPrintView, 800);
        }
    }
});
</script>
...
@media print {
    ...
    .sidebar, .top-navbar, .navbar, .sidenav, header, footer,
    .no-print, .modal, .modal-backdrop, .sidebar-overlay, nav,
    #mainDetailView {
    ...
    #dedicatedPrintView {
        ...
        height: auto !important; overflow: visible !important; z-index: 1 !important;
    }
    #dedicatedPrintPaper {
        position: relative !important; width: 100% !important;
        max-width: 100% !important; margin: 0 !important; padding: 0 !important;
        box-shadow: none !important; border: none !important; border-radius: 0 !important;
        background: #ffffff !important; height: auto !important; overflow: visible !important;
    }
    .doc-page-sheet {
        ...
        box-sizing: border-box !important; min-height: auto !important;
