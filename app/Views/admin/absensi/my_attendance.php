...
<!-- Custom CSS khusus absensi admin -->
<style>
/* Loading Screen */
#loadingScreen {
    position: fixed;
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h1 class="h3 mb-0 text-gray-800">
                <i class="fas fa-clock me-2"></i>Absensi Admin
            </h1>
...
async function getLocationName(latitude, longitude) {
    try {
        const response = await fetch('<?= base_url("admin/absensi/getLocationInfo"); ?>', {
            method: 'POST',
            headers: {
...
async function proceedCheckIn() {
    ...
    try {
        ...
        const response = await fetch('<?= base_url("admin/absensi/checkin"); ?>', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-Requested-With': 'XMLHttpRequest'
        ...
        if (errorMessage.includes('GPS') || errorMessage.includes('lokasi')) {
            ...
            solution = 'Periksa konfigurasi route dan controller.';
        }
...
async function confirmCheckOut() {
    ...
    try {
        ...
        const response = await fetch('<?= base_url("admin/absensi/checkout"); ?>', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-Requested-With': 'XMLHttpRequest'
        ...
        if (errorMessage.includes('Network') || errorMessage.includes('fetch')) {
            ...
            solution = 'Silakan login ulang.';
        } else if (errorMessage.includes('404')) {
            errorMessage = 'Endpoint tidak ditemukan';
            solution = 'Periksa konfigurasi route dan controller.';
        } else if (errorMessage.includes('500')) {
            errorMessage = 'Server error';
            solution = 'Hubungi administrator sistem.';
        } else if (errorMessage.includes('GPS') || errorMessage.includes('lokasi')) {
            solution = 'Aktifkan GPS, berikan izin lokasi, dan pastikan di area terbuka.';
        } else if (errorMessage.includes('Indonesia')) {
            solution = 'Pastikan Anda berada dalam wilayah Indonesia saat absensi.';
        }
        
        showError('Check Out Gagal', errorMessage, solution);
