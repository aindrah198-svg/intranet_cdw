...
<style>
    ...
    .doc-page-break::before {
        ...
        background: #f1f5f9; padding: 4px 16px;
    ...
    .custom-doc-table {
        ...
        margin: 18px 0; font-size: 0.9rem;
    }
    ...
    .content-block-card {
        ...
        margin-bottom: 10px; overflow: hidden; transition: box-shadow 0.2s ease;
    }
    ...
    .page-break-indicator {
        margin: 20px 0 0 0; border-top: 2.5px dashed #6366f1;
        position: relative; display: block; clear: both;
    }
    .page-break-indicator::before {
        content: attr(data-label);
        display: block; text-align: center; font-size: 0.7rem; font-weight: 700;
        color: #6366f1; letter-spacing: 1.5px; background: #eef2ff;
        padding: 3px 14px; border-radius: 20px; width: fit-content;
        margin: -13px auto 12px auto; border: 1.5px solid #c7d2fe;
    }
    .mini-page-header-repeat {
        padding: 8px 0 6px 0; border-bottom: 1.5px solid #334155;
...
<div class="container-fluid py-3 py-md-4">
    ...
    <div class="d-flex justify-content-between align-items-center bg-white rounded-3 shadow-sm p-3 mb-4 border border-light">
        ...
        <a href="<?= base_url('direktur/karyawan/surat/detail/' . $surat['id']) ?>" class="btn btn-outline-secondary rounded-pill px-3.5 py-2 shadow-sm d-inline-flex align-items-center text-sm fw-semibold">
            <i class="fas fa-arrow-left me-1.5"></i> <span class="d-none d-md-inline">Kembali ke Detail</span><span class="d-inline d-md-none">Kembali</span>
        </a>
    </div>
    ...
    <div class="row g-4">
        ...
        <div class="col-12 col-xl-5">
            <div class="card employee-card-modern p-4 mb-4">
                ...
                <form action="<?= base_url('direktur/karyawan/surat/update/' . $surat['id']) ?>" method="post" id="suratForm">
                    <?= csrf_field() ?>
                    ...
                    <div class="row g-3 mb-3">
                        ...
                        <div class="col-md-7">
                            ...
                            <select name="template_layout" id="templateLayout" class="form-select form-select-custom fw-bold text-primary" onchange="applyPresetTemplate()">
                                ...
                                <option value="accent_yellow" <?= ($surat['template_layout'] ?? '') == 'accent_yellow' ? 'selected' : '' ?>>✨ Preset 2: Aksen Kuning Modern (Footer Alamat - Sample 2)</option>
                                <option value="blue_header" <?= ($surat['template_layout'] ?? '') == 'blue_header' ? 'selected' : '' ?>>🔷 Preset 3: Bilah Biru Formal (Sample 3 & 4)</option>
                        ...
                        <div class="col-md-5">
                            ...
                            <select name="paper_size" id="paperSize" class="form-select form-select-custom fw-bold text-dark" onchange="renderLivePreview()">
                    ...
                    <div class="row g-3 mb-3">
                        ...
                        <div class="col-6">
                            ...
                            <select name="address_position" id="addressPosition" class="form-select form-select-custom" onchange="renderLivePreview()">
                                ...
                                <option value="footer" <?= ($surat['address_position'] ?? '') == 'footer' ? 'selected' : '' ?>>Footer Bawah (Kiri)</option>
                                <option value="none" <?= ($surat['address_position'] ?? '') == 'none' ? 'selected' : '' ?>>Sembunyikan Alamat</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <select name="accent_style" id="accentStyle" class="form-select form-select-custom" onchange="renderLivePreview()">
                            ...
                            <option value="yellow_corner" <?= ($surat['accent_style'] ?? '') == 'yellow_corner' ? 'selected' : '' ?>>Aksen Kuning Corner (Sudut Atas & Bawah)</option>
                    ...
                    <?php
                        $isCustomJenis = !in_array($surat['jenis_surat'], $jenisList);
                    ?>
                    ...
                    <div class="row g-3 mb-3">
                        <div class="col-md-6">
                            ...
                            <div id="jenisSuratLainnyaWrapper" class="mt-2 <?= $isCustomJenis ? '' : 'd-none' ?>">
                                ...
                                <input type="text" id="jenisSuratLainnya" name="jenis_surat_custom" class="form-control form-control-custom text-uppercase fw-bold border-primary" placeholder="Misal: SURAT TEGURAN LISAN / BAST..." value="<?= $isCustomJenis ? esc($surat['jenis_surat']) : '' ?>" oninput="renderLivePreview()">
                            </div>
                        </div>
                        <div class="col-md-6">
                    ...
                    <div class="row g-3 mb-3">
                        ...
                        <div class="col-md-5">
                            ...
                            <input type="text" name="perihal" id="perihalInput" class="form-control form-control-custom" placeholder="Contoh: Surat Peringatan Pertama (SP-1)" value="<?= esc($surat['perihal']) ?>" required oninput="renderLivePreview()">
                        </div>
                        <div class="col-md-3">
                            ...
                            <select name="status" id="statusSurat" class="form-select form-select-custom" onchange="renderLivePreview()">
                                ...
                                <option value="diterbitkan" <?= $surat['status'] == 'diterbitkan' ? 'selected' : '' ?>>Terbitkan Sekarang</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <!-- Hidden fields: assembled from blocks before submit -->
                    <input type="hidden" name="isi_surat" id="isiSurat" value="<?= esc($surat['isi_surat']) ?>">
                    <input type="hidden" name="html_full" id="htmlFull" value="<?= esc($surat['html_full']) ?>">
                    <input type="hidden" name="print_now" id="printNowFlag" value="0">

                    <div class="mb-4">
                        <label class="form-label text-sm fw-semibold text-dark">Catatan Internal <span class="text-xs text-muted">(Opsional)</span></label>
                        <textarea name="catatan" id="catatanInput" class="form-control form-control-custom" rows="2" placeholder="Catatan internal arsip..." oninput="renderLivePreview()"><?= esc($surat['catatan'] ?? '') ?></textarea>
                    </div>

                    <!-- Action Buttons -->
                    <div class="pt-3 border-top d-flex justify-content-end gap-2">
                        <a href="<?= base_url('direktur/karyawan/surat/detail/' . $surat['id']) ?>" class="btn btn-light rounded-pill px-4 fw-semibold border">Batal</a>
                        <button type="submit" class="btn btn-primary rounded-pill px-4 py-2 fw-semibold shadow-sm">
                            <i class="fas fa-save me-1.5"></i> Simpan Perubahan Surat
                        </button>
                    </div>
...
<!-- Data Base64 Logo & Nomor Surat dari Server -->
<script>
...
// =====================================================================
// CONTENT BLOCKS SYSTEM (shared with tambah.php)
// =====================================================================
let blocks = [];
let blockCounter = 0;
const PAPER_BODY_PX = {
...
function addBlock(type) {
    ...
    if (type === 'text') { blocks.push({ id, type: 'text', content: '' }); }
        ...
        blocks.push({ id, type: 'table', style: 'blue_header',
            headers: ['No', 'Deskripsi / Uraian', 'Keterangan'],
            rows: [['1','',''],['2','',''],['3','','']] });
    }
    renderBlocksEditor(); renderLivePreview();
    setTimeout(() => { const last = document.querySelector('#contentBlocksEditor .content-block-card:last-child'); if (last) last.scrollIntoView({ behavior:'smooth', block:'nearest' }); }, 100);
}
...
function addTableRow(id) {
    ...
    const row=new Array(b.headers.length).fill(''); row[0]=String(b.rows.length+1); b.rows.push(row);
    renderBlockEditor(id); renderLivePreview();
}
...
function addTableCol(id) {
    ...
    if(b&&b.headers.length<8){b.headers.push('Kolom '+(b.headers.length+1)); b.rows.forEach(r=>r.push('')); renderBlockEditor(id); renderLivePreview();}
}
function removeTableCol(id) {
    const b=blocks.find(b=>b.id===id);
    if(b&&b.headers.length>1){b.headers.pop(); b.rows.forEach(r=>r.pop()); renderBlockEditor(id); renderLivePreview();}
}
...
function buildBlockHtml(b, idx) {
    const first=idx===0; const last=idx===blocks.length-1;
    if (b.type==='text') {
        return `<div class="content-block-card" data-block-id="${b.id}">
            ...
            <div class="block-body-area">
                <textarea class="form-control block-textarea" rows="4"
                    placeholder="Tulis paragraf atau teks di sini..."
                    oninput="updateBlockText('${b.id}',this.value)">${escHtml(b.content)}</textarea>
            </div>
        </div>`;
    }
    if (b.type==='table') {
        const styleOpts=[['blue_header','🔷 Biru Formal'],['gold_header','🟡 Emas Premium'],['standard','⬜ Standard'],['striped','🟦 Striped']]
        ...
        return `<div class="content-block-card table-block-card" data-block-id="${b.id}">
            ...
            <div class="block-body-area table-block-body">
                ...
                <small class="text-muted mt-1 d-block" style="font-size:0.7rem"><i class="fas fa-info-circle me-1"></i>${b.rows.length} baris × ${b.headers.length} kolom</small>
            </div>
        </div>`;
    }
    return '';
...
function renderBlocksEditor() {
    ...
    if(!container) return;
    if(blocks.length===0){
        container.innerHTML=`<div class="block-empty-state"><i class="fas fa-layer-group fa-2x mb-2" style="color:#c7d2fe"></i><p class="mb-1 fw-bold" style="color:#6366f1">Belum ada konten surat</p><p class="mb-0 text-sm">Klik <strong>+ Paragraf</strong> untuk menulis teks, atau <strong>+ Tabel</strong> untuk membuat tabel.</p></div>`;
        return;
    }
...
function updateKaryawanText() {
    ...
    if(opt&&opt.value){
        const divisi=opt.dataset.divisi||'-'; const jabatan=opt.dataset.jabatan||'-';
...
function renderLivePreview() {
    ...
    let jenis=jenisSel;
    if(jenisSel==='Lainnya'){const cv=(document.getElementById('jenisSuratLainnya')||{value:''}).value.trim(); jenis=cv?cv.toUpperCase():'SURAT KARYAWAN';}
    ...
    let cornerAccents='';
    if(accent==='yellow_corner') cornerAccents=`<svg style="position:absolute;top:0;left:0;width:200px;height:200px;pointer-events:none;z-index:1;opacity:0.85;" viewBox="0 0 280 280"><path d="M 0 0 L 280 0 Q 70 70 0 280 Z" fill="#f5a600"/></svg><svg style="position:absolute;bottom:0;right:0;width:160px;height:160px;pointer-events:none;z-index:1;opacity:0.85;" viewBox="0 0 260 260"><path d="M 260 260 L 0 260 Q 190 190 260 0 Z" fill="#f5a600"/></svg>`;

    let footerAddrHtml='';
    if(addrPos==='footer'||accent==='yellow_corner') footerAddrHtml=`<div style="position:absolute;bottom:15px;left:30px;right:180px;font-size:0.7rem;color:#475569;line-height:1.35;z-index:5;"><strong>PT. Cipta Duta Wacana</strong><br>Beltway Office Park Tower B Lt.5, Jl. Letjen TB Simatupang No.41, Ragunan, Jakarta Selatan 12550<br>Tel: +62-21 29857462 | <span style="color:#0284c7">www.cdw-engineering.com</span></div>`;
    ...
    measurer.style.cssText=`position:absolute;visibility:hidden;left:-9999px;top:0;width:${paperWidthPx-120}px;font-family:Inter,Arial,sans-serif;font-size:0.9rem;`;
    ...
    const pages=[[]]; let curH=0; let pNum=1;
    blocks.forEach((b,i)=>{
    ...
    const signatureHtml=`<div class="row pt-4 text-center mt-auto" style="position:relative;z-index:5;margin-bottom:${(addrPos==='footer'||accent==='yellow_corner')?'70px':'15px'};"><div class="col-6 ms-auto"><p class="text-sm mb-5">Hormat kami,<br><strong>PT. CIPTA DUTA WACANA</strong></p><p class="fw-bold text-dark mb-0 border-bottom d-inline-block pb-1" style="min-width:160px;">Direktur Utama</p></div></div>`;
    ...
    pages.forEach((pageBlocks, pi) => {
        ...
        let pageNumIndicator = `<div style="position:absolute;bottom:18px;right:30px;font-size:0.75rem;font-weight:700;color:#64748b;z-index:5;">Halaman ${pageNum} dari ${pages.length}</div>`;
        ...
        if (isFirstPage) {
            paperHtml += `<div style="min-height:120px;margin-bottom:12px;">${kopHtml}</div>`;
            ...
            paperHtml += `<div class="d-flex justify-content-between text-sm fw-semibold text-secondary mb-3 pb-2 border-bottom border-light"><span><strong>Nomor:</strong> ${nomorSuratText}</span><span><strong>Tanggal:</strong> ${dateFormatted}</span></div>`;
    ...
    if (blocks.length === 0) {
        paperHtml = `<div class="doc-page-sheet">
            <div style="position:relative;z-index:2;display:flex;flex-direction:column;flex-grow:1;">
                <div style="min-height:120px;margin-bottom:12px;">${kopHtml}</div>
                ${accentHtml}
                <div style="min-height:200px;display:flex;align-items:center;justify-content:center;color:#94a3b8;"><div class="text-center"><i class="fas fa-file-alt fa-3x mb-3" style="opacity:0.3;"></i><p class="text-sm">Konten surat akan muncul di sini.</p></div></div>
...
document.addEventListener('DOMContentLoaded', function () {
    // Load existing isi_surat into blocks
    const parsed = parseHtmlToBlocks(existingIsiSurat);
    if (parsed.length > 0) {
        blocks = parsed;
    } else {
        blocks = [{ id: genBlkId(), type: 'text', content: '' }];
    }
    renderBlocksEditor();

    // Assemble isi_surat before submit
