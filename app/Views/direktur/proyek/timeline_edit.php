<?php
// app/Views/direktur/proyek/timeline_edit.php

$title = $title ?? 'Edit Project Timeline';
$templateData = [
    'title' => $title,
    'active' => 'proyek'
];
        ...
        <div>
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb mb-1 text-xs">
                    ...
                    <li class="breadcrumb-item active text-primary fw-bold" aria-current="page">Edit Timeline Proyek</li>
                </ol>
            </nav>
            <h4 class="fw-bold text-dark mb-0"><i class="fas fa-edit text-warning me-2"></i> Edit Data Timeline Proyek</h4>
            <small class="text-muted">Perbarui nama proyek, client, jadwal tanggal mulai & estimasi selesai, serta status pelaksanaan.</small>
        </div>
        <div>
        ...
        <div class="col-lg-9">
            <div class="card border-0 rounded-4 shadow-lg overflow-hidden mb-4">
                <div class="card-header bg-gradient-primary text-white py-3 px-4 d-flex justify-content-between align-items-center">
                    <h5 class="card-title fs-6 fw-bold mb-0"><i class="fas fa-edit me-2"></i> Edit Timeline: <?= esc($p['nama_project']) ?></h5>
                    <span class="badge bg-white text-primary rounded-pill px-3 py-1 text-xs fw-bold">Kode: <?= esc($p['kode_project']) ?></span>
                </div>
                <form action="<?= base_url('direktur/proyek/timeline/update') ?>" method="POST" onsubmit="const btn = this.querySelector('button[type=submit]'); if(btn){ btn.disabled = true; btn.innerHTML = '<i class=\'fas fa-spinner fa-spin me-1\'></i> Menyimpan...'; }">
                        ...
                        <div class="row g-3 mb-3">
                            <div class="col-md-6">
                                ...
                                <select class="form-select rounded-3" name="status" required>
                                    ...
                                    <option value="selesai" <?= strtolower($p['status']) == 'selesai' ? 'selected' : '' ?>>Selesai (Tuntas)</option>
                                    <option value="batal" <?= strtolower($p['status']) == 'batal' ? 'selected' : '' ?>>Batal</option>
                                </select>
                            </div>
                            <div class="col-md-6">
