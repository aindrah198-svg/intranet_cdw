...
<div class="container-fluid py-3 py-md-4 fade-in pb-5">
    ...
    <div class="d-flex justify-content-between align-items-center bg-white rounded-3 shadow-sm p-3 mb-4 border border-light">
        <div class="d-flex align-items-center">
            ...
            <div>
                <h4 class="mb-0 fw-bold text-dark">Project Baru</h4>
                <small class="text-muted d-none d-sm-inline">Kelola inisiasi project (Penawaran, Nego, Deal)</small>
            </div>
        </div>
        <button type="button" class="btn btn-primary rounded-pill px-3.5 py-2 shadow-sm d-inline-flex align-items-center text-sm fw-semibold" data-bs-toggle="modal" data-bs-target="#tambahProjectModal">
            <i class="fas fa-plus me-1.5"></i> <span class="d-none d-md-inline">Buat Project Baru</span><span class="d-inline d-md-none">Buat</span>
        </button>
    </div>
    ...
    <div class="input-group mb-4 shadow-sm rounded-pill overflow-hidden border border-light">
        ...
        <input type="text" id="searchProject" class="form-control border-start-0 border-end-0 ps-1 py-2.5" placeholder="Cari nama project, kode, atau status...">
    </div>
    ...
    <!-- Project List Grid (Modern Cards) -->
    <div class="row g-3" id="projectCardContainer">
        <?php if (empty($projects)): ?>
            <div class="col-12 text-center py-5">
                <div class="text-muted mb-3"><i class="fas fa-project-diagram fa-4x opacity-25"></i></div>
                <h6 class="fw-bold text-dark mb-1">Tidak Ada Data Project Baru</h6>
                <p class="text-muted small mb-3">Klik tombol di atas untuk mulai membuat project baru.</p>
            </div>
        <?php else: ?>
            <?php foreach ($projects as $p): ?>
                <?php
                    $status = strtolower($p['status']);
                    $statusPillClass = 'status-pill-penawaran';
                    $statusIcon = 'fas fa-file-contract';
                    $statusLabel = 'Penawaran';

                    if ($status === 'nego') {
                        $statusPillClass = 'status-pill-nego';
                        $statusIcon = 'fas fa-handshake';
                        $statusLabel = 'Negosiasi';
                    } elseif ($status === 'deal') {
                        $statusPillClass = 'status-pill-deal';
                        $statusIcon = 'fas fa-check-double';
                        $statusLabel = 'Deal / Sepakat';
                    }
                    
                    $initial = strtoupper(substr($p['nama_project'], 0, 1));
                ?>
                <div class="col-12 col-xl-6 project-card-wrapper" data-status="<?= esc($status) ?>">
                    <div class="card employee-card-modern p-3 p-sm-4 h-100">
                                ...
                                <div class="col-sm-4">
                                    <div class="data-pill-bar">
                                        ...
                                        <div class="data-value" style="font-weight: 500;">
                                            <?= $p['tanggal_mulai'] ? date('d M Y', strtotime($p['tanggal_mulai'])) : '<i class="text-muted">Belum Ditentukan</i>' ?>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-sm-4">
                        ...
                        <div class="pt-3 mt-auto border-top border-light d-flex align-items-center justify-content-end gap-2 flex-wrap">
                            ...
                            <button type="button" onclick="confirmDeleteProyek(<?= $p['id'] ?>, '<?= esc($p['nama_project'], 'js') ?>')" class="btn-action-pill text-danger border-danger bg-danger bg-opacity-10" title="Hapus Project">
...
<div class="modal fade" id="tambahProjectModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-lg modal-dialog-centered">
        <div class="modal-content border-0 rounded-4 shadow-lg">
            <div class="modal-header bg-gradient-primary text-white rounded-top-4 py-3 px-4">
                <h5 class="modal-title fs-6 fw-bold mb-0"><i class="fas fa-project-diagram me-2"></i> Buat Project Baru</h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form action="<?= base_url('direktur/proyek/simpan') ?>" method="POST">
                <div class="modal-body p-4">
                    <div class="row g-3 mb-3">
                        <div class="col-md-6">
                            ...
                            <small class="text-muted text-xs">Pilih dari daftar proyek terdahulu atau ketik nama proyek baru.</small>
                        </div>
                        <div class="col-md-6">
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control rounded-3" name="deskripsi" rows="3" placeholder="Rincian singkat mengenai scope kerja project..."></textarea>
                    </div>
                    ...
                    <div class="row g-3 mb-3">
                        ...
                        <div class="col-md-4">
                            ...
                            <input type="date" class="form-control rounded-3" name="tanggal_selesai">
                        </div>
                    </div>
                    ...
                    <div class="row g-3 mb-3">
                        <div class="col-md-6">
                            ...
                            <select class="form-select rounded-3" name="status" required>
                                ...
                                <option value="nego">Nego / Negosiasi</option>
                                <option value="deal">Deal / Setuju</option>
                            </select>
                        </div>
                        <div class="col-md-6">
                ...
                <div class="modal-footer bg-light rounded-bottom-4 py-2.5 px-4">
                    ...
                    <button type="submit" class="btn btn-primary rounded-pill px-4 fw-semibold shadow-sm">
                        <i class="fas fa-save me-1.5"></i> Simpan Project
                    </button>
                </div>
            </form>
        </div>
...
<script>
    // Live Format Rupiah as User Types
    ...
    function submitQuickClient(e) {
        ...
        .then(data => {
            ...
            if (data.status === 'success') {
                // Add new client to select dropdown & select it
                ...
                Swal.fire({
                    ...
                    text: 'Client "' + data.nama_perusahaan + '" telah ditambahkan dan terpilih.',
                    timer: 2500,
                ...
                Swal.fire({
                    icon: 'error',
                    title: 'Gagal!',
                    text: data.message || 'Gagal menambahkan client.',
                    confirmButtonColor: '#0d6efd'
                });
            }
        })
        .catch(err => {
            ...
            Swal.fire({
                icon: 'error',
                title: 'Error!',
                text: 'Terjadi kesalahan sistem saat menyimpan client.',
                confirmButtonColor: '#0d6efd'
            });
        });
    }

    function confirmDeleteProyek(id, nama) {
        Swal.fire({
            title: 'Hapus Project?',
            text: 'Project "' + nama + '" akan dihapus permanen dari sistem.',
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                ...
                form.method = 'POST';
                form.action = '<?= base_url('direktur/proyek/delete') ?>/' + id;
