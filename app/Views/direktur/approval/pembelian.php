<?php
// app/Views/direktur/approval/pembelian.php
...
$baseUrl = $baseUrl ?? base_url('direktur/approval/pembelian');
$queryParams = $queryParams ?? [];
$stats = $stats ?? ['total_pr' => 0, 'total_menunggu' => 0, 'total_disetujui' => 0, 'total_ditolak' => 0, 'total_urgent' => 0, 'total_tinggi' => 0, 'total_nominal' => 0];
...
<div class="container-fluid px-4 py-4">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h4 class="mb-1 text-gradient">
                <i class="fas fa-shopping-cart me-2"></i>Approval Pembelian
            </h4>
            <p class="text-muted mb-0">Persetujuan Purchase Request (PR) oleh Direktur</p>
        </div>
        <div>
            ...
            <button class="btn btn-modern-outline me-2" onclick="window.location.href='<?= base_url('direktur/approval/pembelian/export-excel?' . http_build_query(array_filter($queryParams))) ?>'">
    ...
    <div class="row g-3 mb-4">
        ...
        <div class="col-md-3 col-sm-6">
            <div class="modern-card modern-card-danger text-center">
                ...
                <h2 class="mb-0"><?= number_format(($stats['total_urgent'] ?? 0) + ($stats['total_tinggi'] ?? 0)) ?></h2>
                <p class="text-muted mb-0">Prioritas Tinggi/Urgent</p>
            </div>
        </div>
        <div class="col-md-3 col-sm-6">
            <div class="modern-card modern-card-warning text-center">
                ...
                <p class="text-muted mb-0">Total Estimasi</p>
                <small class="text-muted"><?= number_format($stats['total_pr'] ?? 0) ?> PR</small>
            </div>
        </div>
    </div>
    ...
    <div class="modern-card mb-4">
        ...
        <form method="GET" action="<?= base_url('direktur/approval/pembelian') ?>" class="row g-3" id="filterForm">
            <div class="col-md-2">
                ...
                <select name="status" class="form-select" onchange="this.form.submit()">
                    ...
                    <option value="ordered" <?= $statusFilter === 'ordered' ? 'selected' : '' ?>>Dipesan</option>
            ...
            <div class="col-md-2">
                ...
                <select name="prioritas" class="form-select" onchange="this.form.submit()">
                    ...
                    <?php foreach ($prioritasList as $key => $value): ?>
                    <option value="<?= $key ?>" <?= $prioritasFilter === $key ? 'selected' : '' ?>><?= $value ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            <div class="col-md-2">
            ...
            <div class="col-md-4">
                ...
                <input type="text" name="search" class="form-control" placeholder="No. PR / Nama / Supplier..." value="<?= htmlspecialchars($searchQuery) ?>">
            </div>
            <div class="col-md-12 d-flex gap-2">
                <button type="submit" class="btn btn-primary">
                    <i class="fas fa-search me-2"></i>Filter
                </button>
                <a href="<?= base_url('direktur/approval/pembelian') ?>" class="btn btn-secondary">
                ...
                <tbody>
                    ...
                    <?php else: ?>
                        ...
                        <tr class="<?= $isPending ? 'table-warning' : '' ?>">
                            ...
                            <td><?= $no++ ?></td>
                            <td>
                                <strong><?= htmlspecialchars($item['nomor_pr']) ?></strong>
                            </td>
                            ...
                            <td><?= getPrioritasBadge($item['prioritas'] ?? 'Normal') ?></td>
                            <td><?= formatCurrency($item['total_estimasi'] ?? 0) ?></td>
                            <td>
                                <?= htmlspecialchars(substr($item['supplier'] ?? '-', 0, 30)) ?>
                            </td>
                            <td><?= getDirekturStatusBadge($item['status_direktur'] ?? 'Menunggu') ?></td>
                            <td>
                                <a href="<?= base_url('direktur/approval/pembelian/detail/' . $item['id']) ?>" 
                                   class="btn btn-sm btn-outline-primary" title="Detail">
                                    <i class="fas fa-eye"></i>
                                </a>
                                <?php if ($isPending): ?>
                                <button type="button" class="btn btn-sm btn-success approve-btn" data-id="<?= $item['id'] ?>" data-nomor="<?= htmlspecialchars($item['nomor_pr']) ?>" data-nama="<?= htmlspecialchars($item['nama_panggilan'] ?? $item['nama_lengkap']) ?>" title="Setujui">
                                    <i class="fas fa-check"></i>
                                </button>
                                <button type="button" class="btn btn-sm btn-danger reject-btn" data-id="<?= $item['id'] ?>" data-nomor="<?= htmlspecialchars($item['nomor_pr']) ?>" data-nama="<?= htmlspecialchars($item['nama_panggilan'] ?? $item['nama_lengkap']) ?>" title="Tolak">
                                    <i class="fas fa-times"></i>
                                </button>
                                <?php endif; ?>
                                <a href="<?= base_url('direktur/approval/pembelian/print/' . $item['id']) ?>" 
...
$(document).ready(function() {
    ...
    $('#batchApproveBtn').on('click', function() {
        ...
        Swal.fire({
            title: 'Konfirmasi',
            text: 'Anda akan menyetujui ' + selectedIds.length + ' purchase request?',
            icon: 'question',
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '<?= base_url('direktur/approval/pembelian/batch-approve') ?>',
                    type: 'POST',
