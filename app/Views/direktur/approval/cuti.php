<?php
// app/Views/direktur/approval/cuti.php

$cutiData = $cutiData ?? [];
$karyawanList = $karyawanList ?? [];
$jenisCutiList = $jenisCutiList ?? ['Tahunan', 'Sakit', 'Hamil', 'Penting', 'Izin', 'Lainnya'];
...
$baseUrl = $baseUrl ?? base_url('direktur/approval/cuti');
$queryParams = $queryParams ?? [];
$stats = $stats ?? ['total' => 0, 'menunggu' => 0, 'disetujui' => 0, 'ditolak' => 0, 'total_hari_cuti' => 0];
...
<div class="container-fluid px-4 py-4">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h4 class="mb-1 text-gradient">
                <i class="fas fa-calendar-check me-2"></i>Approval Cuti Karyawan
            </h4>
            <p class="text-muted mb-0">Persetujuan pengajuan cuti karyawan oleh Direktur</p>
        </div>
        <div>
            ...
            <button class="btn btn-modern-outline me-2" onclick="window.location.href='<?= base_url('direktur/approval/cuti/export-excel?' . http_build_query(array_filter($queryParams))) ?>'">
    ...
    <div class="row g-3 mb-4">
        <div class="col-md-3 col-sm-6">
            <div class="modern-card modern-card-primary text-center">
                ...
                <h2 class="mb-0"><?= number_format($stats['menunggu'] ?? 0) ?></h2>
                <p class="text-muted mb-0">Menunggu Persetujuan</p>
                <?php if (($stats['menunggu'] ?? 0) > 0): ?>
                    <small class="text-warning">Perlu tindakan segera</small>
                <?php endif; ?>
            </div>
        </div>
        <div class="col-md-3 col-sm-6">
            <div class="modern-card modern-card-accent text-center">
                ...
                <h2 class="mb-0"><?= number_format($stats['disetujui'] ?? 0) ?></h2>
        ...
        <div class="col-md-3 col-sm-6">
            <div class="modern-card modern-card-danger text-center">
                ...
                <h2 class="mb-0"><?= number_format($stats['ditolak'] ?? 0) ?></h2>
        ...
        <div class="col-md-3 col-sm-6">
            <div class="modern-card modern-card-warning text-center">
                ...
                <small class="text-muted"><?= number_format($stats['total_hari_cuti'] ?? 0) ?> hari cuti</small>
            </div>
        </div>
    </div>
    ...
    <div class="modern-card mb-4">
        ...
        <form method="GET" action="<?= base_url('direktur/approval/cuti') ?>" class="row g-3" id="filterForm">
            <div class="col-md-2">
            ...
            <div class="col-md-2">
                ...
                <select name="jenis" class="form-select" onchange="this.form.submit()">
                    ...
                    <?php foreach ($jenisCutiList as $jenis): ?>
                    <option value="<?= $jenis ?>" <?= $jenisFilter === $jenis ? 'selected' : '' ?>><?= $jenis ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            <div class="col-md-2">
            ...
            <div class="col-md-3">
                ...
                <input type="text" name="search" class="form-control" placeholder="Nama / NIK / No. Cuti..." value="<?= htmlspecialchars($searchQuery) ?>">
            </div>
    ...
    <div class="modern-card">
        ...
        <div class="table-responsive">
            <table class="table table-hover" id="cutiTable">
                <thead>
                    <tr>
                        <th width="40">
                ...
                <tbody>
                    ...
                    <?php else: ?>
                        ...
                        foreach ($cutiData as $item): 
                            $isPending = ($item['status_direktur'] === 'Menunggu' && $item['status_hrd'] === 'Disetujui');
                        ?>
                        <tr class="<?= $isPending ? 'table-warning' : '' ?>">
                            ...
                            <td><?= $no++ ?></td>
                            <td>
                                <strong><?= htmlspecialchars($item['nomor_cuti']) ?></strong>
                            </td>
                            ...
                            <td><?= getJenisCutiBadge($item['jenis_cuti']) ?></td>
                            <td>
                                <span class="fw-bold"><?= $item['lama_hari'] ?></span> hari
                            </td>
                            <td>
                                <?php if ($item['status_atasan'] === 'Disetujui'): ?>
                                    <span class="badge bg-success">Disetujui</span>
                                <?php elseif ($item['status_atasan'] === 'Ditolak'): ?>
                            ...
                            <td>
                                <?php if ($item['status_hrd'] === 'Disetujui'): ?>
                                    <span class="badge bg-success">Disetujui</span>
                            ...
                            <td><?= getStatusBadge($item['status_direktur']) ?></td>
                            <td>
                                <a href="<?= base_url('direktur/approval/cuti/detail/' . $item['id']) ?>" 
                                   class="btn btn-sm btn-outline-primary" title="Detail">
                                    <i class="fas fa-eye"></i>
                                </a>
                                <?php if ($isPending): ?>
                                <button type="button" class="btn btn-sm btn-success approve-btn" data-id="<?= $item['id'] ?>" title="Setujui">
                                    <i class="fas fa-check"></i>
                                </button>
                                <button type="button" class="btn btn-sm btn-danger reject-btn" data-id="<?= $item['id'] ?>" data-nama="<?= htmlspecialchars($item['nama_panggilan'] ?? $item['nama_lengkap']) ?>" title="Tolak">
                                    <i class="fas fa-times"></i>
                                </button>
                                <?php endif; ?>
                                <a href="<?= base_url('direktur/approval/cuti/print/' . $item['id']) ?>" 
...
$(document).ready(function() {
    ...
    $('#batchApproveBtn').on('click', function() {
        ...
        Swal.fire({
            title: 'Konfirmasi',
            text: 'Anda akan menyetujui ' + selectedIds.length + ' pengajuan cuti?',
            icon: 'question',
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '<?= base_url('direktur/approval/cuti/batch-approve') ?>',
                    type: 'POST',
