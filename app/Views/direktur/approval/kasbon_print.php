<?php
// app/Views/direktur/approval/kasbon_print.php
...
<head>
    <meta charset="UTF-8">
    <title>Form Kasbon - <?= htmlspecialchars($kasbon['nomor_kasbon'] ?? 'Pengajuan Kasbon') ?></title>
    <style>
        * {
            margin: 0;
            padding: 0;
    ...
    <div class="print-container">
        ...
        <div class="no-print" style="margin-bottom: 20px; text-align: center;">
            <button onclick="window.print();" class="print-btn">
                <i class="fas fa-print"></i> Cetak / Salin PDF
            </button>
        ...
        <!-- Title -->
        <div class="title">
            <h3>FORMULIR PENGAJUAN KASBON</h3>
            <p>No. <?= htmlspecialchars($kasbon['nomor_kasbon'] ?? '-') ?></p>
        </div>

        <!-- Surat Info -->
        <div class="surat-info">
            Jakarta, <?= formatDateIndo($kasbon['tanggal_pengajuan'] ?? date('Y-m-d')) ?>
        </div>
        ...
        <div class="section">
            ...
            <table class="info-table">
                <tr>
                    <td>NIK / NIP</td>
                    <td>: <?= htmlspecialchars($kasbon['nik'] ?? '-') ?></td>
                </tr>
                <tr>
                    <td>Nama Lengkap</td>
                    <td>: <?= htmlspecialchars($kasbon['nama_lengkap'] ?? '-') ?></td>
                </tr>
                <tr>
                    <td>Jabatan</td>
                    <td>: <?= htmlspecialchars($kasbon['jabatan'] ?? '-') ?></td>
                </tr>
                <tr>
                    <td>Departemen</td>
                    <td>: <?= htmlspecialchars($kasbon['departemen'] ?? '-') ?></td>
                </tr>
                <tr>
                    <td>Tanggal Masuk</td>
                    <td>: <?= formatDateIndo($kasbon['karyawan_tanggal_masuk'] ?? '') ?></td>
                </tr>
            </table>
        </div>
        ...
        <!-- Detail Kasbon -->
        <div class="section">
            <div class="section-title">B. DETAIL KASBON</div>
            <table class="info-table">
                <tr>
                    <td>Jumlah Kasbon</td>
                    <td>: <?= formatCurrency($kasbon['jumlah_kasbon'] ?? 0) ?></td>
                </tr>
                <tr>
                    <td>Terbilang</td>
                    <td>: <strong><?= terbilang(floor($kasbon['jumlah_kasbon'] ?? 0)) ?> Rupiah</strong></td>
                </tr>
                <tr>
                    <td>Tanggal Dibutuhkan</td>
                    <td>: <?= !empty($kasbon['tanggal_dibutuhkan']) ? formatDateIndo($kasbon['tanggal_dibutuhkan']) : '-' ?></td>
                </tr>
                <tr>
                    <td>Alasan Kasbon</td>
                    <td>: <?= nl2br(htmlspecialchars($kasbon['alasan'] ?? '-')) ?></td>
                </tr>
                <tr>
                    <td>Rencana Pelunasan</td>
                    <td>: <?= nl2br(htmlspecialchars($kasbon['rencana_pelunasan'] ?? '-')) ?></td>
                </tr>
            </table>
        </div>
            ...
            <table class="info-table">
                <tr>
                    ...
                    <td>: 
                        ...
                        <?php endif; ?>
                        <?php if (!empty($kasbon['hrd_nama']) && ($kasbon['status_hrd'] ?? '') === 'Disetujui HRD'): ?>
                            <br><small>Oleh: <?= htmlspecialchars($kasbon['hrd_nama']) ?></small>
                        <?php endif; ?>
                    </td>
                </tr>
                <tr>
                    ...
                    <td>: 
                        <?php if (($kasbon['status_direktur'] ?? '') === 'Disetujui'): ?>
                            <span class="status-badge status-approved">DISETUJUI</span>
                        <?php elseif (($kasbon['status_direktur'] ?? '') === 'Ditolak'): ?>
                            <span class="status-badge status-rejected">DITOLAK</span>
                        <?php else: ?>
                            <span class="status-badge status-pending">MENUNGGU</span>
                        <?php endif; ?>
                        <?php if (!empty($kasbon['direktur_nama']) && ($kasbon['status_direktur'] ?? '') !== 'Menunggu'): ?>
                            <br><small>Oleh: <?= htmlspecialchars($kasbon['direktur_nama']) ?></small>
                            <br><small>Tanggal: <?= formatDateTime($kasbon['disetujui_direktur_at'] ?? '') ?></small>
                        <?php endif; ?>
                    </td>
                </tr>
                <tr>
                    <td>Status Keseluruhan</td>
                    <td>: <strong><?= getKeseluruhanStatusText($kasbon['status_keseluruhan'] ?? 'Draft') ?></strong></td>
                </tr>
                <?php if (!empty($kasbon['alasan_penolakan_direktur'])): ?>
                <tr>
                    <td>Alasan Penolakan</td>
                    <td>: <?= nl2br(htmlspecialchars($kasbon['alasan_penolakan_direktur'])) ?></td>
                </tr>
                <?php endif; ?>
            </table>
        </div>
        ...
        <div class="section">
            <div class="section-title">D. INFORMASI PENCAIRAN</div>
            <table class="info-table">
                <?php if (!empty($kasbon['tanggal_pencairan'])): ?>
                <tr>
                    <td>Tanggal Pencairan</td>
                    <td>: <?= formatDateIndo($kasbon['tanggal_pencairan']) ?></td>
                </tr>
                <?php endif; ?>
                <?php if (!empty($kasbon['metode_pencairan'])): ?>
                <tr>
                    <td>Metode Pencairan</td>
                    <td>: <?= $kasbon['metode_pencairan'] === 'transfer' ? 'Transfer Bank' : 'Tunai' ?></td>
                </tr>
                <?php endif; ?>
                <?php if (!empty($kasbon['bank_tujuan'])): ?>
                <tr>
                    <td>Bank Tujuan</td>
                    <td>: <?= htmlspecialchars($kasbon['bank_tujuan']) ?></td>
                </tr>
                <?php endif; ?>
                <?php if (!empty($kasbon['no_rekening_tujuan'])): ?>
                <tr>
                    <td>No. Rekening Tujuan</td>
                    <td>: <?= htmlspecialchars($kasbon['no_rekening_tujuan']) ?></td>
                </tr>
                <?php endif; ?>
            </table>
        </div>
        ...
        <div class="section">
            <div class="section-title">E. SISA PINJAMAN</div>
            <table class="info-table">
                <tr>
                ...
                <tr>
                    <td>Sisa Pinjaman</td>
                    <td>: <?= formatCurrency($kasbon['sisa_pinjaman'] ?? 0) ?></td>
                </tr>
                <?php if (!empty($kasbon['lunas_pada'])): ?>
                <tr>
                    <td>Lunas Pada</td>
                    <td>: <?= formatDateIndo($kasbon['lunas_pada']) ?></td>
                </tr>
                <?php endif; ?>
            </table>
        </div>
        ...
        <div class="signature">
            <div class="signature-box">
                ...
                <div class="signature-title">(<?= htmlspecialchars($kasbon['nama_panggilan'] ?? $kasbon['nama_lengkap'] ?? '_________________') ?>)</div>
            </div>
            <div class="signature-box">
                <div class="signature-line"></div>
                <div class="signature-name">HRD,</div>
                <div class="signature-title">(<?= htmlspecialchars($kasbon['hrd_nama'] ?? '_________________') ?>)</div>
            </div>
            <div class="signature-box">
                <div class="signature-line"></div>
                <div class="signature-name">Direktur,</div>
                <div class="signature-title">(<?= htmlspecialchars($kasbon['direktur_nama'] ?? 'Cecep Trihardiyanto') ?>)</div>
            </div>
        </div>
        ...
        <div class="footer">
            ...
            Formulir Kasbon ini adalah dokumen resmi PT. Cipta Duta Wacana (CDW Engineering)
        </div>
    </div>
