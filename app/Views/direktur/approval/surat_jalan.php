<?php
// app/Views/direktur/approval/surat_jalan.php
...
$baseUrl = $baseUrl ?? base_url('direktur/approval/surat-jalan');
$queryParams = $queryParams ?? [];
$stats = $stats ?? ['total_surat_jalan' => 0, 'total_menunggu' => 0, 'total_disetujui' => 0, 'total_ditolak' => 0, 'total_dikirim' => 0, 'total_diterima' => 0];
...
<div class="container-fluid px-4 py-4">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h4 class="mb-1 text-gradient">
                <i class="fas fa-truck me-2"></i>Approval Surat Jalan
            </h4>
            <p class="text-muted mb-0">Persetujuan Surat Jalan Pengiriman Barang oleh Direktur</p>
        </div>
        <div>
            ...
            <button class="btn btn-modern-outline me-2" onclick="window.location.href='<?= base_url('direktur/approval/surat-jalan/export-excel?' . http_build_query(array_filter($queryParams))) ?>'">
    ...
    <div class="row g-3 mb-4">
        ...
        <div class="col-md-3 col-sm-6">
            <div class="modern-card modern-card-warning text-center">
                ...
                <h2 class="mb-0"><?= number_format(($stats['total_dikirim'] ?? 0) + ($stats['total_diterima'] ?? 0)) ?></h2>
                <p class="text-muted mb-0">Dikirim / Diterima</p>
            </div>
        </div>
        <div class="col-md-3 col-sm-6">
            <div class="modern-card modern-card-danger text-center">
                ...
                <h2 class="mb-0"><?= number_format($stats['total_surat_jalan'] ?? 0) ?></h2>
                <p class="text-muted mb-0">Total Surat Jalan</p>
            </div>
        </div>
    </div>
    ...
    <div class="modern-card mb-4">
        ...
        <form method="GET" action="<?= base_url('direktur/approval/surat-jalan') ?>" class="row g-3" id="filterForm">
            <div class="col-md-2">
            ...
            <div class="col-md-4">
                ...
                <input type="text" name="search" class="form-control" placeholder="No. Surat Jalan / Client / Project..." value="<?= htmlspecialchars($searchQuery) ?>">
            </div>
            <div class="col-md-12 d-flex gap-2">
                <button type="submit" class="btn btn-primary">
                    <i class="fas fa-search me-2"></i>Filter
                </button>
                <a href="<?= base_url('direktur/approval/surat-jalan') ?>" class="btn btn-secondary">
            ...
            <table class="table table-hover">
                ...
                <tbody>
                    ...
                    <?php else: ?>
                        ...
                        foreach ($suratJalanData as $item): 
                        ...
                        <tr class="<?= $isPending ? 'table-warning' : '' ?>">
                            ...
                            <td><?= $no++ ?></td>
                            <td>
                                <strong><?= htmlspecialchars($item['nomor_surat_jalan']) ?></strong>
                            </td>
                            <td><?= formatDate($item['tanggal_kirim'] ?? '') ?></td>
                            <td>
                                <?= htmlspecialchars($item['nama_project'] ?? '-') ?>
                            </td>
                            <td>
                                <strong><?= htmlspecialchars($item['client_nama'] ?? '-') ?></strong>
                            </td>
                            <td>
                                <?= htmlspecialchars($item['penerima_nama'] ?? '-') ?>
                            </td>
                            <td><?= getStatusPengirimanBadge($item['status'] ?? 'diproses') ?></td>
                            <td><?= getDirekturStatusBadge($item['status_direktur'] ?? 'Menunggu') ?></td>
                            <td>
                                <a href="<?= base_url('direktur/approval/surat-jalan/detail/' . $item['id']) ?>" 
                                   class="btn btn-sm btn-outline-primary" title="Detail">
                                    <i class="fas fa-eye"></i>
                                </a>
                                <?php if ($isPending): ?>
                                <button type="button" class="btn btn-sm btn-success approve-btn" data-id="<?= $item['id'] ?>" data-nomor="<?= htmlspecialchars($item['nomor_surat_jalan']) ?>" title="Setujui">
                                    <i class="fas fa-check"></i>
                                </button>
                                <button type="button" class="btn btn-sm btn-danger reject-btn" data-id="<?= $item['id'] ?>" data-nomor="<?= htmlspecialchars($item['nomor_surat_jalan']) ?>" title="Tolak">
                                    <i class="fas fa-times"></i>
                                </button>
                                <?php endif; ?>
                                <a href="<?= base_url('direktur/approval/surat-jalan/print/' . $item['id']) ?>" 
...
$(document).ready(function() {
    ...
    $('#batchApproveBtn').on('click', function() {
        ...
        Swal.fire({
            title: 'Konfirmasi',
            text: 'Anda akan menyetujui ' + selectedIds.length + ' surat jalan?',
            icon: 'question',
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '<?= base_url('direktur/approval/surat-jalan/batch-approve') ?>',
                    type: 'POST',
