<?php
// app/Views/direktur/approval/bast.php
...
$baseUrl = $baseUrl ?? base_url('direktur/approval/bast');
$queryParams = $queryParams ?? [];
$stats = $stats ?? ['total_bast' => 0, 'total_menunggu' => 0, 'total_disetujui' => 0, 'total_ditolak' => 0, 'total_kondisi_baik' => 0, 'total_kondisi_cukup' => 0, 'total_kondisi_perlu_perbaikan' => 0];
...
<div class="container-fluid px-4 py-4">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h4 class="mb-1 text-gradient">
                <i class="fas fa-file-signature me-2"></i>Approval BAST
            </h4>
            <p class="text-muted mb-0">Persetujuan Berita Acara Serah Terima oleh Direktur</p>
        </div>
        <div>
            ...
            <button class="btn btn-modern-outline me-2" onclick="window.location.href='<?= base_url('direktur/approval/bast/export-excel?' . http_build_query(array_filter($queryParams))) ?>'">
    ...
    <div class="row g-3 mb-4">
        ...
        <div class="col-md-3 col-sm-6">
            <div class="modern-card modern-card-warning text-center">
                ...
                <h2 class="mb-0"><?= number_format(($stats['total_kondisi_baik'] ?? 0) + ($stats['total_kondisi_cukup'] ?? 0) + ($stats['total_kondisi_perlu_perbaikan'] ?? 0)) ?></h2>
                <p class="text-muted mb-0">Total BAST</p>
                <small class="text-muted"><?= number_format($stats['total_bast'] ?? 0) ?> dokumen</small>
            </div>
        </div>
        <div class="col-md-3 col-sm-6">
            <div class="modern-card modern-card-danger text-center">
                ...
                <h2 class="mb-0"><?= number_format($stats['total_kondisi_baik'] ?? 0) ?> / <?= number_format($stats['total_kondisi_perlu_perbaikan'] ?? 0) ?></h2>
                <p class="text-muted mb-0">Baik / Perlu Perbaikan</p>
            </div>
        </div>
    </div>
    ...
    <div class="modern-card mb-4">
        ...
        <form method="GET" action="<?= base_url('direktur/approval/bast') ?>" class="row g-3" id="filterForm">
            <div class="col-md-2">
            ...
            <div class="col-md-2">
                ...
                <select name="kondisi" class="form-select" onchange="this.form.submit()">
                    ...
                    <option value="<?= $key ?>" <?= $kondisiFilter === $key ? 'selected' : '' ?>><?= $value ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            <div class="col-md-2">
            ...
            <div class="col-md-4">
                ...
                <input type="text" name="search" class="form-control" placeholder="No. BAST / Client / Judul..." value="<?= htmlspecialchars($searchQuery) ?>">
            </div>
            <div class="col-md-12 d-flex gap-2">
                <button type="submit" class="btn btn-primary">
                    <i class="fas fa-search me-2"></i>Filter
                </button>
                <a href="<?= base_url('direktur/approval/bast') ?>" class="btn btn-secondary">
    ...
    <div class="modern-card">
        <div class="d-flex justify-content-between align-items-center mb-3">
            <h5 class="mb-0">
                ...
                Daftar Berita Acara Serah Terima (BAST)
                ...
                <tbody>
                    ...
                    <?php else: ?>
                        ...
                        <tr class="<?= $isPending ? 'table-warning' : '' ?>">
                            ...
                            <td><?= $no++ ?></td>
                            <td>
                                <strong><?= htmlspecialchars($item['nomor_bast']) ?></strong>
                            </td>
                            <td><?= formatDate($item['tanggal_bast'] ?? '') ?></td>
                            <td>
                                <div>
                            ...
                            <td><?= getKondisiBadge($item['kondisi'] ?? 'Baik') ?></td>
                            <td><?= htmlspecialchars($item['nomor_spk'] ?? '-') ?></td>
                            <td><?= getDirekturStatusBadge($item['status_direktur'] ?? 'Menunggu') ?></td>
                            <tr>
                                <a href="<?= base_url('direktur/approval/bast/detail/' . $item['id']) ?>" 
                                   class="btn btn-sm btn-outline-primary" title="Detail">
                                    <i class="fas fa-eye"></i>
                                </a>
                                <?php if ($isPending): ?>
                                <button type="button" class="btn btn-sm btn-success approve-btn" data-id="<?= $item['id'] ?>" data-nomor="<?= htmlspecialchars($item['nomor_bast']) ?>" title="Setujui">
                                    <i class="fas fa-check"></i>
                                </button>
                                <button type="button" class="btn btn-sm btn-danger reject-btn" data-id="<?= $item['id'] ?>" data-nomor="<?= htmlspecialchars($item['nomor_bast']) ?>" title="Tolak">
                                    <i class="fas fa-times"></i>
                                </button>
                                <?php endif; ?>
                                <a href="<?= base_url('direktur/approval/bast/print/' . $item['id']) ?>" 
...
$(document).ready(function() {
    ...
    $('#batchApproveBtn').on('click', function() {
        ...
        Swal.fire({
            title: 'Konfirmasi',
            text: 'Anda akan menyetujui ' + selectedIds.length + ' BAST?',
            icon: 'question',
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '<?= base_url('direktur/approval/bast/batch-approve') ?>',
                    type: 'POST',
