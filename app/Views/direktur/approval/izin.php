<?php
// app/Views/direktur/approval/izin.php
...
$baseUrl = $baseUrl ?? base_url('direktur/approval/izin');
$queryParams = $queryParams ?? [];
$stats = $stats ?? ['total_izin' => 0, 'total_menunggu' => 0, 'total_disetujui' => 0, 'total_ditolak' => 0, 'total_sakit' => 0, 'total_hari_izin' => 0];
...
<div class="container-fluid px-4 py-4">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h4 class="mb-1 text-gradient">
                <i class="fas fa-user-edit me-2"></i>Approval Izin
            </h4>
            <p class="text-muted mb-0">Persetujuan Pengajuan Izin Karyawan oleh Direktur</p>
        </div>
        <div>
            ...
            <button class="btn btn-modern-outline me-2" onclick="window.location.href='<?= base_url('direktur/approval/izin/export-excel?' . http_build_query(array_filter($queryParams))) ?>'">
    ...
    <div class="row g-3 mb-4">
        ...
        <div class="col-md-3 col-sm-6">
            <div class="modern-card modern-card-warning text-center">
                ...
                <h2 class="mb-0"><?= number_format($stats['total_hari_izin'] ?? 0) ?></h2>
                <p class="text-muted mb-0">Total Hari Izin</p>
                <small class="text-muted"><?= number_format($stats['total_izin'] ?? 0) ?> pengajuan</small>
            </div>
        </div>
    </div>
    ...
    <div class="modern-card mb-4">
        ...
        <form method="GET" action="<?= base_url('direktur/approval/izin') ?>" class="row g-3" id="filterForm">
            <div class="col-md-2">
            ...
            <div class="col-md-2">
                ...
                <select name="jenis" class="form-select" onchange="this.form.submit()">
                    ...
                    <?php foreach ($jenisIzinList as $key => $value): ?>
            ...
            <div class="col-md-4">
                ...
                <input type="text" name="search" class="form-control" placeholder="No. Izin / Nama / NIK..." value="<?= htmlspecialchars($searchQuery) ?>">
            </div>
            <div class="col-md-12 d-flex gap-2">
                <button type="submit" class="btn btn-primary">
                    <i class="fas fa-search me-2"></i>Filter
                </button>
                <a href="<?= base_url('direktur/approval/izin') ?>" class="btn btn-secondary">
                ...
                <tbody>
                    ...
                    <?php else: ?>
                        ...
                        foreach ($izinData as $item): 
                            $isPending = ($item['status_keseluruhan'] === 'Menunggu' && $item['status_hrd'] === 'Disetujui');
                        ?>
                        <tr class="<?= $isPending ? 'table-warning' : '' ?>">
                            ...
                            <td><?= $no++ ?></td>
                            <td>
                                <strong><?= htmlspecialchars($item['nomor_izin']) ?></strong>
                            </td>
                            ...
                            <td><?= getJenisIzinBadge($item['jenis_izin']) ?></td>
                            <td>
                            ...
                            <td><?= getStatusBadge($item['status_keseluruhan'] ?? 'Menunggu') ?></td>
                            <td>
                                <a href="<?= base_url('direktur/approval/izin/detail/' . $item['id']) ?>" 
                                   class="btn btn-sm btn-outline-primary" title="Detail">
                                    <i class="fas fa-eye"></i>
                                </a>
                                <?php if ($isPending): ?>
                                <button type="button" class="btn btn-sm btn-success approve-btn" data-id="<?= $item['id'] ?>" data-nomor="<?= htmlspecialchars($item['nomor_izin']) ?>" data-nama="<?= htmlspecialchars($item['nama_panggilan'] ?? $item['nama_lengkap']) ?>" title="Setujui">
                                    <i class="fas fa-check"></i>
                                </button>
                                <button type="button" class="btn btn-sm btn-danger reject-btn" data-id="<?= $item['id'] ?>" data-nomor="<?= htmlspecialchars($item['nomor_izin']) ?>" data-nama="<?= htmlspecialchars($item['nama_panggilan'] ?? $item['nama_lengkap']) ?>" title="Tolak">
                                    <i class="fas fa-times"></i>
                                </button>
                                <?php endif; ?>
                                <a href="<?= base_url('direktur/approval/izin/print/' . $item['id']) ?>" 
...
$(document).ready(function() {
    ...
    $('#batchApproveBtn').on('click', function() {
        ...
        Swal.fire({
            title: 'Konfirmasi',
            text: 'Anda akan menyetujui ' + selectedIds.length + ' pengajuan izin?',
            icon: 'question',
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '<?= base_url('direktur/approval/izin/batch-approve') ?>',
                    type: 'POST',
