   ...
   public function index()
{
 ...
 /**
     * 🔥 PERBAIKAN: Tampilkan form konfirmasi posting
     * (Method ini sebenarnya sudah ada, tapi pastikan parameter dikirim dengan benar)
     */
    public function postAllJurnalsForm()
    {
        ...
        $isAlreadyPosted = $this->bukuBesarModel->isPeriodeAlreadyPosted($periode);
        if ($isAlreadyPosted) {
            $postedJurnals = $this->bukuBesarModel->getPostedJurnalIdsInPeriode($periode);
            $postedList = [];
            foreach ($postedJurnals as $j) {
                $postedList[] = $j['nomor_jurnal'];
            }
            
            return redirect()->to(site_url('accounting/pembukuan/buku-besar'))
                ->with('error', 'Periode ' . $this->getNamaBulan($bulan) . ' ' . $tahun . ' sudah pernah diposting! Jurnal berikut sudah ada di buku besar: ' . implode(', ', $postedList));
        }
        ...
        if (empty($pendingJurnals)) {
            // Cek apakah ada jurnal di periode tersebut (tapi sudah diposting atau memang kosong)
            $db = \Config\Database::connect();
            $startDate = $periode . '-01';
            $endDate = date('Y-m-t', strtotime($periode . '-01'));
            
            $jurnalCount = $db->table('jurnal')
            ...
            if ($jurnalCount > 0) {
                return redirect()->to(site_url('accounting/pembukuan/buku-besar'))
                    ->with('error', 'Semua jurnal periode ' . $this->getNamaBulan($bulan) . ' ' . $tahun . ' sudah diposting ke buku besar.');
            } else {
                return redirect()->to(site_url('accounting/pembukuan/buku-besar'))
                    ->with('error', 'Tidak ada jurnal untuk periode ' . $this->getNamaBulan($bulan) . ' ' . $tahun . '. Silakan buat jurnal terlebih dahulu.');
            }
        }
      ...
      public function postAllJurnals()
    {
        ...
        if ($isAlreadyPosted) {
            return $this->response->setJSON([
                'success' => false,
                'message' => 'Periode ' . $this->getNamaBulan($bulan) . ' ' . $tahun . ' sudah pernah diposting!',
                'already_posted' => true
            ]);
        }
        
        // 🔥 CEK APAKAH ADA JURNAL DENGAN STATUS 'POSTED' UNTUK PERIODE INI
        $db = \Config\Database::connect();
        $startDate = $periode . '-01';
        $endDate = date('Y-m-t', strtotime($startDate));
        
        $postedJurnalCount = $db->table('jurnal')
            ->where('tanggal >=', $startDate)
            ->where('tanggal <=', $endDate)
            ->where('status', 'posted')
            ->countAllResults();
        
        if ($postedJurnalCount === 0) {
            return $this->response->setJSON([
                'success' => false,
                'message' => 'Tidak ada jurnal dengan status POSTED pada periode ' . $this->getNamaBulan($bulan) . ' ' . $tahun . '. Pastikan jurnal sudah difinalisasi (status=posted) sebelum diposting ke buku besar.',
                'no_posted_jurnal' => true
            ]);
        }
        ...
        if (empty($pendingJurnals)) {
            return $this->response->setJSON([
                'success' => false,
                'message' => 'Semua jurnal periode ' . $this->getNamaBulan($bulan) . ' ' . $tahun . ' sudah diposting ke buku besar.'
            ]);
        }
...
private function generateNeracaSaldoPdfHtml($neracaSaldo)
{
   ...
   /**
     * AJAX: Get pending counts berdasarkan bulan dan tahun
     * 🔥 FIX: Sekarang benar-benar memfilter berdasarkan periode
     */
    public function ajaxGetPendingCounts()
    {
        ...
        // 🔥 VALIDASI: Wajib ada bulan dan tahun
        if (empty($bulan) || empty($tahun)) {
            return $this->response->setJSON([
                'success' => false,
                'message' => 'Bulan dan tahun wajib dipilih',
        ...
        log_message('debug', '[BukuBesar] ajaxGetPendingCounts - periode: ' . $periode);
        
        // 🔥 PERBAIKAN: Gunakan method getPendingCountsByMonth dari model
        $counts = $this->bukuBesarModel->getPendingCountsByMonth($bulan, $tahun);
        ...
        return $this->response->setJSON([
            'success' => true,
            'jurnal_count' => $counts['jurnal_count'],
            'detail_count' => $counts['detail_count'],
            'periode' => $periode,
            'already_posted' => $isAlreadyPosted
        ]);
    }
...
private function generateBatchPdfHtml($data, $batchId)
{
 ...
 /**
     * 🔥 PERBAIKAN: AJAX Get Available Periods (periode yang memiliki jurnal pending)
     */
    public function ajaxGetAvailablePeriods()
    {
        ...
        // 🔥 PERBAIKAN: Ambil periode dari jurnal yang belum diproses
        // Gunakan query yang lebih akurat
        $processedSubquery = $db->table('buku_besar')
            ->select('DISTINCT jurnal_detail_id')
            ->where('is_void', 0)
            ->get();
        ...
        $builder = $db->table('jurnal_detail')
            ->select('DISTINCT jurnal.tanggal')
        ...
        foreach ($result as $row) {
            $periode = date('Y-m', strtotime($row['tanggal']));
